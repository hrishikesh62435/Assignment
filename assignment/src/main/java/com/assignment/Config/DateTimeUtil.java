...
public class DateTimeUtil {
	...
	public static String formatScientificNumber(String number) {
        // Convert to a string and split into mantissa and exponent parts String numberStr = Double.toString(number);
        
        String[] parts = number.split("[Ee]");

        if (parts.length == 2) {
            // Format the mantissa part with commas
            double mantissa = Double.parseDouble(parts[0]);
            String formattedMantissa = String.format("%,.2f", mantissa);

            // Combine the formatted mantissa and exponent with "e"
            return formattedMantissa + "e" + parts[1];
        } else {
